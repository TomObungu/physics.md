...
g = \frac{F}{m}
$$
$F$ is the gravitational force and $m$ is the mass of the object in the gravitational field. Gravitational field strength has the unit $Nkg^{-1}$.

This unit is the same as $ms^{-1}$ which the SI unit for acceleration. This is because $F=mg$ as the same as $F=ma$. In other words, the gravitational field strength at a point is the same as the acceleration of free fall of an object on that point. If $F=mg=ma$, then $g = \frac{ma}{m}$, therefore $g=a$.  

On the surface of the Earth the gravitational field strength is approximately equal to $9.81Nkg^{-1}$
